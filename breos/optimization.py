...
from breos.solar import calculate_pv_production_dc, PVModuleParams, default_azimuth
...
def optimize_slope(
    ...
    slope_range: Tuple[float, float] = (0.0, 60.0),
    ...
    Args:
        ...
        slope_range: (min_slope, max_slope) in degrees
        objective: 'max_production' or 'max_self_consumption'
        freq: Time frequency
        n_points: Number of slope values to evaluate
        verbose: Print progress
        
    Returns:
        OptimizationResult with optimal slope
    """
    if surface_azimuth is None:
        surface_azimuth = default_azimuth(location.latitude)
    slopes = np.linspace(slope_range[0], slope_range[1], n_points)
    results = []
    ...
    for slope in slopes:
        try:
            ...
            results.append({'slope': slope, 'production_kwh': total_production})
            
            if verbose:
                print(f"  Slope {slope:.1f}°: {total_production:.1f} kWh")
                
        except Exception as e:
            if verbose:
                print(f"  Slope {slope:.1f}°: Error - {e}")
            results.append({'slope': slope, 'production_kwh': 0})
    
    results_df = pd.DataFrame(results)
    optimal_idx = results_df['production_kwh'].idxmax()
    optimal_slope = results_df.loc[optimal_idx, 'slope']
    optimal_production = results_df.loc[optimal_idx, 'production_kwh']
    
    if verbose:
        print(f"\nOptimal slope: {optimal_slope:.1f}° ({optimal_production:.1f} kWh)")
    
    return OptimizationResult(
        optimal_value=optimal_slope,
        objective_value=optimal_production,
        iterations=len(slopes),
        details={'all_results': results_df}
    )


def optimize_slope_brent(
    weather_data: pd.DataFrame,
    location,
    n_modules: int,
    pv_params: Optional[PVModuleParams] = None,
    ...
    def objective(slope):
        ...
        try:
            ...
            if verbose:
                print(f"  Iteration {iterations[0]}: slope={slope:.2f}°, production={-production:.1f} kWh")
...
def size_for_zeb(
    ...
    if yearly_pv <= 0:
        return {
            'error': 'No PV production',
            'modules_needed': float('inf')
        }
...
def calculate_financials(
    ...
    """
    if costs_config is None: costs_config = {}
    if financials_config is None: financials_config = {}
    ...
    class SolarDesignProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs):
            ...
            if params:
                ...
                module_area = pv_spec.get('dimensions', {}).get('width', 1.134) * \
                              pv_spec.get('dimensions', {}).get('length', 2.278)
            else:
            ...
            capex, npv = calculate_financials(
                n_modules, battery_kwh, total_import, total_export, total_load,
