...
class NCA_GrSi_SonyMurata2p5Ah_Battery(BatteryDegradationModel):
    ...
    def update_rates(self, stressors):
        ...
        pcal = np.trapezoid(pcal, x=t_secs) / delta_t_secs
        kcyc = np.trapezoid(kcyc, x=t_secs) / delta_t_secs
        pcyc = np.trapezoid(pcyc, x=t_secs) / delta_t_secs
    ...
    def update_states(self, stressors):
        ...
        # Capacity
        dq_t = self._degradation_scalar * self._update_sigmoid_state(
            states["qLoss_t"][-1], delta_t_days / 5e3, r["kcal"], p["q2"], r["pcal"]
        )
        dq_EFC = self._degradation_scalar * self._update_sigmoid_state(
            states["qLoss_EFC"][-1], delta_efc / 1e5, r["kcyc"], p["q4"], r["pcyc"]
        )
    ...
    def _extract_stressors(self, t_secs, soc, T_celsius):
        ...
        if np.isnan(Crate):
            ...
            if len(instantaneous_cdis) > 1:
                Cdis = np.trapezoid(np.abs(instantaneous_cdis), t_secs_discharging) / (
                    t_secs_discharging[-1] - t_secs_discharging[0]
                )
