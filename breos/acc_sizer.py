...
def run_acc_sizer_logic(config: Dict[str, Any], verbose: bool = True) -> pd.DataFrame:
    ...
    location = Location(
        loc_cfg["latitude"], loc_cfg["longitude"], tz=loc_cfg.get("timezone", "UTC"), name=loc_cfg.get("name", "")
    )

    # Weather
    if verbose:
        print("Fetching TMY Data...")
    tmy_data, _ = fetch_tmy_weather_data(loc_cfg["latitude"], loc_cfg["longitude"], freq="h")

    # Households (Load)
    households = config.get("households", [])
    if verbose:
        print(f"Loading profiles for {len(households)} households...")
    load_matrix = []
    ...
    base_pv_curve = calculate_multi_array_production(
        weather_data=tmy_data, location=location, arrays=base_arrays, freq="h", verbose=False
    )
