...
def create_cost_plots(
    ...
    if 'Cost_No_Sys_Cumulative_NPV' in cost_projection.columns:
        ax.plot(cost_projection['Year'], cost_projection['Cost_No_Sys_Cumulative_NPV'], 
                'r--', label='No System (NPV)', linewidth=2)
        ax.plot(cost_projection['Year'], cost_projection['Cost_System_Cumulative_NPV'], 
                'g-', label='With PV System (NPV)', linewidth=2)
    else:
        ax.plot(cost_projection['Year'], cost_projection['Cost_No_Sys_Cumulative'], 
                'r--', label='No System', linewidth=2)
        ax.plot(cost_projection['Year'], cost_projection['Cost_System_Cumulative'], 
                'g-', label='With PV System', linewidth=2)
    ...
    if 'Savings_Cumulative_NPV' in cost_projection.columns:
        ...
        if not payback.empty:
            ...
            ax.annotate(f'Payback: Year {payback_year}', 
                       xy=(payback_year, ax.get_ylim()[1]*0.9),
...
def monthly_graphs(
    ...
    for i, (col, color, label) in enumerate(zip(columns, colors, labels)):
        if col in monthly.columns:
            ax.bar([xi + i*width for xi in x], monthly[col], width, 
...
def weekly_graphs(
    ...
    if 'PV_Production' in week_data.columns:
        ax.fill_between(week_data.index, 0, week_data['PV_Production']/1000, 
                       alpha=0.3, color='gold', label='PV Production')
    if 'Houseload' in week_data.columns:
        ax.plot(week_data.index, week_data['Houseload']/1000, 
               'b-', label='Load', linewidth=1.5)
    if 'Battery_Energy' in week_data.columns:
        ax2 = ax.twinx()
        ax2.plot(week_data.index, week_data['Battery_Energy']/1000, 
                'g--', label='Battery (kWh)', linewidth=1.5)
...
def degradation_plots(
    ...
    if 'Global_Cycle_Degradation' in degradation_df.columns:
        ...
        _plot_degradation_components(glob_cyc, glob_cal, 'battery_degradation_components_global.png', 'Global Cumulative Degradation (%)')
    ...
    if 'Cumulative_Cycle_Degradation' in degradation_df.columns:
        ...
        _plot_degradation_components(cum_cyc, cum_cal, 'battery_degradation_components_per_battery.png', 'Per-Battery Cumulative Degradation (%)')
...
def plot_validation_parity(
    ...
    if metrics and 'R2' in metrics:
        ax.text(0.05, 0.95, f"R\u00b2 = {metrics['R2']:.4f}", transform=ax.transAxes,
...
def plot_validation_multi_system(
    ...
    for i, (sid, res) in enumerate(sorted(systems_results.items())):
        ...
        # Predicted as line
        ax.plot(sim['date'], sim['predicted_soh'], '-', color=color,
                linewidth=1.2, alpha=0.7)

        # Measured as markers
        if not truth.empty:
            ax.scatter(truth['date'], truth['measured_soh'], color=color,
...
def plot_validation_degradation_split(
    ...
    ax.fill_between(dates, cal_loss, cal_loss + cycle_loss, alpha=0.6,
                    color='#FF5722', label='Cycle aging')
...
def plot_cell_temperature(
    ...
    ax.fill_between(months, min_by_month.values, max_by_month.values,
                    alpha=0.25, color='red', label='Min–Max range')
    ax.plot(months, mean_by_month.values, 'r-o', linewidth=1.5,
...
def plot_breakeven(
    ...
    ax1.plot(years, no_sys, 'r-', linewidth=2.5, marker='o', markersize=4, 
             label=f'No System{label_suffix}')
    ax1.plot(years, with_sys, 'g-', linewidth=2.5, marker='s', markersize=4, 
             label=f'PV System{label_suffix}')
...
def plot_slope_optimization(
    ...
    if optimal_marker:
        ...
        ax.scatter([optimal_x], [optimal_y], s=200, c='red', marker='*', 
                  edgecolor='black', linewidth=1.5, zorder=5, label=f'Optimal: {optimal_x}°')
        ax.annotate(f'{optimal_y:.1f} kWh', 
...
def plot_monthly_comparison(
    ...
    colors = {'PV_Production': '#FFD700', 'Houseload': '#4169E1', 
              'Import_From_Grid': '#FF6347', 'Sell_To_Grid': '#32CD32'}
    labels = {'PV_Production': 'PV Generation', 'Houseload': 'Load Demand',
              'Import_From_Grid': 'Grid Import', 'Sell_To_Grid': 'Grid Export'}
    
    for i, col in enumerate(columns):
        ax.bar(x + i*width, monthly[col], width, 
               label=labels.get(col, col), color=colors.get(col, 'gray'), 
    ...
    for i, col in enumerate(columns):
        for j, val in enumerate(monthly[col]):
            if val > 0:
                ax.text(j + i*width, val + monthly[col].max()*0.01, 
...
def plot_breakeven_distribution(
    ...
    n, bins, patches = ax.hist(breakeven_steps, bins=bins, color='skyblue', 
    ...
    for i in range(len(n)):
        if n[i] > 0:
            ax.text((bins[i] + bins[i+1])/2, n[i] + 0.1, int(n[i]), 
...
def plot_breakeven_cdf(
    ...
    for q, color in zip(quantiles, colors):
        ...
        ax.text(val, q, f" {q:.1%} ({val:.1f}y)", color=color, 
...
def plot_montecarlo_cost_overlay(
    ...
    ax.plot(mean_nosys.index, mean_nosys.values, color='darkred', linewidth=2.5, linestyle='--', label='Mean No System Cost')
...
def plot_montecarlo_soh_overlay(
    all_results_df: pd.DataFrame,
    results_directory: str,
    suffix: str = ""
) -> None:
...
def plot_montecarlo_npv_distribution(
    ...
    for val, label, ls, lw in line_cfg:
        ax.axvline(x=val, color='tab:red', linestyle=ls, linewidth=lw,
                   label=f'{label}: {val:,.0f} EUR')
...
def plot_optimization_results_3d(
    ...
    try:
        ...
        sc = ax.scatter(df[x_col], df[y_col], df[z_col], 
                       c=df[z_col], cmap='viridis', s=80, alpha=0.8, edgecolor='black', linewidth=0.5)
...
def plot_optimization_results_2d(
    ...
    try:
        ...
        sc = plt.scatter(df[x_col], df[y_col], 
                        c=df[z_col], cmap='viridis', s=100, alpha=0.8)
...
def plot_tariff_comparison_manual(
    ...
    try:
        ...
        rects1 = ax.bar(x - width/2, vals_nosys, width, label='No System', color='indianred', alpha=0.8, edgecolor='black')
        rects2 = ax.bar(x + width/2, vals_sys, width, label='With System', color='steelblue', alpha=0.8, edgecolor='black')
...
def plot_tariff_comparison(
    ...
    if 'No System Cost (€)' in results_df.columns and 'Net Cost (€)' in results_df.columns:
        ...
        rects1 = ax.bar(x - width/2, no_sys, width, label='No System', color='lightgray', alpha=1.0, edgecolor='gray', linewidth=0.5)
        
        # With System: Distinct Colors, Hatch pattern
        rects2 = ax.bar(x + width/2, with_sys, width, label='With System', color=sys_colors, alpha=0.9, edgecolor='black', linewidth=0.5, hatch='///')
...
def plot_smart_charging_sweep(
    ...
    if not optimal_row.empty:
        ...
        ax.axvline(x=optimal_pct*100, color='r', linestyle='--', alpha=0.7, label=f'Optimal: {optimal_pct*100:.0f}%')
...
def plot_acc_sizing_sweep(
    ...
    ax2.scatter([opt_fin_x], [opt_fin_y], s=150, c='red', marker='*', edgecolors='black', zorder=10, 
                label='Financial Opt.')
    ...
    ax1.scatter([opt_tech_x], [opt_tech_y], s=150, c='blue', marker='*', edgecolors='black', zorder=10,
                label='Technical Opt.')
    ...
    sc = ax.scatter(results_df['Self_Sufficiency_Pct'], results_df['Payback_Years'], 
                    c=results_df['System_kWp'], cmap='viridis', s=100, edgecolor='black')
    ...
    for idx in [opt_fin_idx, opt_tech_idx]:
        ...
        ax.annotate(txt, (results_df.loc[idx, 'Self_Sufficiency_Pct'], results_df.loc[idx, 'Payback_Years']),
...
def plot_montecarlo_soh_traces(
...
def plot_weather_monthly_comparison(
    ...
    ax.plot(x, hist_min, color="tomato", linewidth=1.5, linestyle="--",
            marker="v", markersize=5, zorder=3, label="Min year")
    ax.plot(x, hist_max, color="seagreen", linewidth=1.5, linestyle="--",
            marker="^", markersize=5, zorder=3, label="Max year")

    # 95% CI shaded band
    ax.fill_between(x, hist_ci_low, hist_ci_high,
                    color="steelblue", alpha=0.25, label="95% CI")

    # Historical mean line
    ax.plot(x, hist_mean, color="steelblue", linewidth=2.5,
            marker="o", markersize=7, zorder=4, label="Historical mean")

    # TMY line (on top)
    ax.plot(x, tmy_vals, color="darkorange", linewidth=2.5,
            marker="D", markersize=6, zorder=6, label=f"TMY ({tmy_source})")
...
def plot_weather_annual_ghi_distribution(
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.hist(annual_ghi_per_year.values, bins=10, color="steelblue",
            edgecolor="black", alpha=0.7, label=f"Historical ({n_years} years)")
    ax.axvline(tmy_annual_ghi, color="darkorange", linewidth=2.5, linestyle="--",
               label=f"TMY ({tmy_annual_ghi:.0f} kWh/m²)")
    ax.axvline(hist_annual_ghi_mean, color="navy", linewidth=2, linestyle="-",
...
def plot_breakeven_comparison(
    ...
    for df, label, color in zip(cost_dfs, labels, colors):
        ...
        if no_sys_values not in seen_baselines:
            ...
            ax.plot(df['Year'], df['Cost_No_Sys_Cumulative_NPV'],
                    color=color, linestyle='--', label=no_sys_label, linewidth=2.5, alpha=0.7)

    max_year = 20
    for df, label, color in zip(cost_dfs, labels, colors):
        ax.plot(df['Year'], df['Cost_System_Cumulative_NPV'],
...
def plot_breakeven_two(
    df1: "pd.DataFrame", label1: str, be1: "Optional[float]",
    df2: "pd.DataFrame", label2: str, be2: "Optional[float]",
    results_dir: str,
    ...
    ax.plot(df1['Year'], df1['Cost_No_Sys_Cumulative_NPV'],
            'r--', label='No System', linewidth=2.5)
    ax.plot(df1['Year'], df1['Cost_System_Cumulative_NPV'],
            'b-', label=label1, linewidth=2)
    ax.plot(df2['Year'], df2['Cost_System_Cumulative_NPV'],
            'g-', label=label2, linewidth=2)
    ...
    if be1:
        ...
        ax.annotate(f'{label1}: {_fmt_years_months(be1)}',
                    xy=(be1 + 0.3, y_pos), fontsize=10, color='blue')
    if be2:
        ax.axvline(x=be2, color='green', linestyle=':', alpha=0.7, linewidth=1.5)
        ax.annotate(f'{label2}: {_fmt_years_months(be2)}',
                    xy=(be2 + 0.3, y_pos * 0.92), fontsize=10, color='green')
...
def plot_azislope_landscape_2d(
    ...
    sc = ax.scatter(df_grid['Azimuth'], df_grid['Slope'],
                    c=df_grid['Metric'], cmap='viridis', s=50)
    ax.scatter([opt_azimuth], [opt_slope],
               color='red', marker='x', s=200, linewidth=3, label='Optimum')
...
def plot_azislope_landscape_3d(
    ...
    ax.scatter([opt_azimuth], [opt_slope], [opt_metric],
               color='red', s=100, label='Optimum', zorder=10)
...
def plot_pareto_front_analysis(
    ...
    for i, cons in enumerate(consumptions):
        ...
        mask = _is_pareto_efficient(subset['Net_Cost_Eur'].values,
                                    subset['Grid_Independence_%'].values)
        pareto_subset = subset[mask].copy()
        pareto_subset.to_csv(
            os.path.join(results_dir, f'pareto_front_{cons}.csv'), index=False
        )
        ...
        for tariff in tariffs:
            for strategy in strategies:
                ...
                if m.any():
                    ax.scatter(subset.loc[m, 'Grid_Independence_%'],
                               subset.loc[m, 'Net_Cost_Eur'],
                               c=color_map[tariff], marker=marker_map[strategy],
                               alpha=0.2, label=None)

        for tariff in tariffs:
            for strategy in strategies:
                m = (pareto_subset['Tariff'] == tariff) & \
                    (pareto_subset['Detailed_Strategy'] == strategy)
                if m.any():
                    ax.scatter(pareto_subset.loc[m, 'Grid_Independence_%'],
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Tariffs:', markersize=0)
    ]
    for tariff, color in color_map.items():
        legend_elements.append(
            Line2D([0], [0], marker='o', color='w', markerfacecolor=color,
                   label=tariff, markersize=10)
        )
    legend_elements.append(
        Line2D([0], [0], marker='o', color='w', label='Strategies:', markersize=0)
    )
    for strategy, marker in marker_map.items():
        legend_elements.append(
            Line2D([0], [0], marker=marker, color='w', markeredgecolor='black',
                   markerfacecolor='gray', label=strategy, markersize=10)
        )
...
def plot_loo_cv_summary(
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bars_train = ax.bar(x - width / 2, train_rmses, width, color='#2196F3',
                        alpha=0.85, label='Train RMSE')
    bars_held = ax.bar(x + width / 2, held_out_rmses, width, color='#F44336',
                       alpha=0.85, label='Held-out RMSE')

    ax.axhline(y=mean_cv, color='#F44336', linestyle='--', linewidth=1.5,
               alpha=0.7, label=f'Mean CV RMSE ({mean_cv:.1f} pp)')
...
def plot_loo_param_stability(
    ...
    for param_key, ylabel, use_log in param_configs:
        ...
        ax.axhline(y=ref_value, color='#F44336', linestyle='--', linewidth=1.5,
                   alpha=0.7, label=f'Full calibration ({ref_value:.3e})')
...
def plot_loo_predictions(
    ...
    for i, sp in enumerate(systems_predictions):
        ...
        ax.plot(sp['dates_predicted'], np.array(sp['soh_predicted']) * 100,
                '-', color=color, linewidth=1.5, alpha=0.8)
        ax.scatter(sp['dates_measured'], np.array(sp['soh_measured']) * 100,
...
def plot_calendar_aging_sensitivity(
    ...
    for i, (label, soh_values) in enumerate(soh_trajectories.items()):
        ...
        ax.plot(years, soh_values, color=color, linewidth=2, marker=marker,
                markersize=5, markevery=max(1, len(years) // 10), label=label, zorder=3)

    # EOL threshold line
    ax.axhline(eol_threshold, color='grey', linewidth=1.5, linestyle='--',
               label=f'EOL threshold ({eol_threshold:.0f}%)', zorder=2)
...
def plot_co2_savings(
    ...
    ax.bar(x - bar_width / 2, co2_total, bar_width, label='Total PV Production',
           color='#2196F3', alpha=0.85)
    ax.bar(x + bar_width / 2, co2_self, bar_width, label='Self-Consumed PV',
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'co2_avoided_yearly{suffix}.png'),
    ...
    ax.plot(years, co2_total_cum / 1000, 'b-', linewidth=2.5, marker='o',
            markersize=4, label='Total PV Production')
    ax.plot(years, co2_self_cum / 1000, 'g-', linewidth=2.5, marker='s',
            markersize=4, label='Self-Consumed PV')
    ax.fill_between(years, 0, co2_self_cum / 1000, alpha=0.15, color='green')
    ax.fill_between(years, co2_self_cum / 1000, co2_total_cum / 1000,
                    alpha=0.10, color='blue')

    # Annotate final values
    final_total = co2_total_cum.iloc[-1] / 1000
    final_self = co2_self_cum.iloc[-1] / 1000
    ax.annotate(f'{final_total:,.1f} t',
                xy=(years.iloc[-1], final_total),
                xytext=(-50, 10), textcoords='offset points',
                fontsize=11, fontweight='bold', color='#1565C0')
    ax.annotate(f'{final_self:,.1f} t',
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'co2_avoided_cumulative{suffix}.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def plot_degradation_methodology_comparison(
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(years_pvbat, soh_pvbat, 'b-', linewidth=2.5, marker='o',
            markersize=3, label='PVBAT (Naumann)')
    ax.plot(years_polysun, soh_polysun, 'r--', linewidth=2.5, marker='s',
            markersize=3, label='Polysun (Miner/Wöhler)')
    ...
    ax.set_ylabel('State of Health (%)', fontsize=12)
    ax.set_ylim(60, 102)
    ax.set_xticks(range(int(min(years_pvbat[0], years_polysun[0])),
                        int(max(years_pvbat[-1], years_polysun[-1])) + 1))
    ax.legend(fontsize=11)
    ax.grid(alpha=0.3)
    if scenario_label:
        ax.text(0.98, 0.02, scenario_label, transform=ax.transAxes, fontsize=10,
                ha='right', va='bottom', style='italic', alpha=0.7)

    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'polysun_pvbat_soh_comparison{suffix}.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)

    # --- Figure 2: Polysun damage accumulation ---
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(years_polysun, polysun_df['Damage_Cumulative'].values, 'r-',
            linewidth=2.5, marker='s', markersize=3, label="Miner's cumulative damage")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'polysun_miner_damage{suffix}.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def plot_lifetime_prediction_comparison(
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bars1 = ax.bar(x - width / 2, pvbat_years, width, label='PVBAT (Naumann)',
                   color='#1976D2', alpha=0.85)
    bars2 = ax.bar(x + width / 2, polysun_years, width, label='Polysun (Miner/Wöhler)',
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'lifetime_prediction_comparison{suffix}.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def plot_temperature_sensitivity_comparison(
    ...
    for i, (bar, temp) in enumerate(zip(bars1, temps)):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                f'{bar.get_height():.1f}y', ha='center', va='bottom', fontsize=9)
    for bar in bars2:
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                f'{bar.get_height():.1f}y', ha='center', va='bottom', fontsize=9)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(results_directory, f'temperature_sensitivity_comparison{suffix}.png'),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
