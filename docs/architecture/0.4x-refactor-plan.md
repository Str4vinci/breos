...
- `breos.degradation.protocol` defines the internal lifecycle `Protocol`,
  normalized daily input/output records, and native/BLAST adapters.
- The battery simulation keeps dispatch, resistance feedback, replacement
  energy, and ledger ownership while calling one lifecycle step/reset/snapshot
  boundary for either engine.
- The App runner uses lifecycle state to construct degradation results without
  separate native/BLAST tuple-unpacking and provenance branches.
- The contract remains internal; model discovery and the `breos.App` facade are
  unchanged.
...
2. Repository-health tooling — refreshed dependencies and GitHub Actions,
   security-focused dependency maintenance, core coverage reporting, and
   lightweight cross-platform smoke tests.
...
4. R4 — narrow internal degradation lifecycle protocol with native and BLAST
   adapters, preserving dispatch and public result schemas.

Planned after the onboarding boundary has settled:

1. R5 — snapshot codecs
2. R6 — degradation session
...
automation, R7, and R4 form the completed 0.4.2 groundwork. R5 and R6 should
wait for 0.4.3 or later. Reassess the plan after each phase rather than assuming
later interfaces before earlier refactors have settled.
