...
def run_chain(d: pd.DataFrame, poa_col: str, temp_model: str, loss_overrides, degradation_pct: float) -> pd.DataFrame:
    ...
    dc_raw = _module_dc_before_losses(np.clip(poa, 0.0, None), temp_cell, MODULE, N_MODULES, d.index, "dc_raw_W")
    _, dc_after, _, _ = _apply_pvwatts_loss_series(dc_raw, loss_overrides, age_degradation_percent=degradation_pct)
    ac_w, _, _ = _calculate_dc_ac_power_arrays(dc_after.to_numpy(), INVERTER_AC_W, INVERTER_ETA)
    return pd.DataFrame(
        {
            "temp_cell": temp_cell,
