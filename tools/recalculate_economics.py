...
_SKIP_PREFIXES = ('_test', 'validation', 'weather_comparison', 'model_comparison',
                  'degradation_comparison', 'degradation_model_comparison',
                  'batch_comparison', 'calendar_sensitivity', 'ea_sensitivity',
                  'montecarlo', 'webapp_test', 'tou_germany_analysis',
                  'optimization_run', 'azislope_optimization')
_SKIP_EXACT = {'comparison', 'joao_tables', 'joao_pv_battery_optimization',
               'tou_reference_case_spain'}
...
def main():
    ...
    parser.add_argument('--joao-only', action='store_true',
                        help='Only process joao_* directories')
    ...
    for d in dirs:
        ...
        icon = '✓' if status.startswith('OK') or status.startswith('DRY') else (
               '·' if status.startswith('SKIP') else '✗')
