#!/usr/bin/env python3
"""
Azimuth and Slope Optimizer
===========================
Optimizes PV system orientation (Azimuth, Slope) for maximum production or grid independence.

Usage:
    python tools/azislope_optimizer.py configs/scenarios/azislope_porto.json
"""
...
from breos.solar import default_azimuth
...
class OptimizerContext:
    ...
    def evaluate(self, x):
        ...
        Standard Mode: x[0] = Azimuth, x[1] = Slope
        East-West Mode: x[0] = Slope (Azimuths fixed at 90 and 270)
        """
        ...
        try:
            ...
            if self.objective_type == 'max_production':
                ...
                results_df, _, summary_df, _, _, _ = simulate_energy_balance(
                    pv_dc=dc_power,
                    houseload=self.load_data,
                    battery_config=final_batt_config,
                    freq='h',
                    debug=False
                )
            ...
            print(f"Error evaluating ({azimuth_repr}, {slope:.2f}): {e}")
...
def main():
    parser = argparse.ArgumentParser(description="Optimize Azimuth and Slope")
    ...
    results_dir = Path(out_cfg.get('folder', 'results/azislope'))
    ...
    tmy_data, meta = fetch_tmy_weather_data(
    ...
    for mode in modes_to_run:
        ...
        slope_bounds = tuple(bounds_cfg.get('slope', [10, 90]))
        
        if mode == 'east_west':
            # One variable: Slope
            bounds = [slope_bounds]
            print(f"Bounds: Slope {slope_bounds} (Fixed Azimuths: 90 / 270)")
        else:
            # Two variables: Azimuth, Slope
            default_azi_bounds = [90, 270] if location.latitude >= 0 else [-90, 90]
            azi_bounds = tuple(bounds_cfg.get('azimuth', default_azi_bounds))
            bounds = [azi_bounds, slope_bounds]
            print(f"Bounds: Azimuth {azi_bounds}, Slope {slope_bounds}")
        ...
        if mode == 'east_west':
            res_data['raw_slope'] = result.x[0]
            res_data['raw_azimuth'] = "East-West"
            
            res_data['opt_slope'] = round(res_data['raw_slope'])
            res_data['discrete_slope'] = round(res_data['opt_slope'] / 5) * 5
            
            # Eval Rounded
            res_data['opt_metric'] = -ctx.evaluate((res_data['opt_slope'],))
            res_data['discrete_metric'] = -ctx.evaluate((res_data['discrete_slope'],))
            ...
            res_data['raw_slope'] = result.x[1]
            
            res_data['opt_azimuth'] = round(res_data['raw_azimuth'])
            res_data['opt_slope'] = round(res_data['raw_slope'])
            res_data['opt_metric'] = -ctx.evaluate((res_data['opt_azimuth'], res_data['opt_slope']))
            
            res_data['discrete_azimuth'] = round(res_data['opt_azimuth'] / 5) * 5
            res_data['discrete_slope'] = round(res_data['opt_slope'] / 5) * 5
            res_data['discrete_metric'] = -ctx.evaluate((res_data['discrete_azimuth'], res_data['discrete_slope']))
        ...
        if mode == 'standard':
            ...
            slope_vals = np.linspace(slope_bounds[0], slope_bounds[1], n_slope)
            ...
            df_grid = pd.DataFrame(grid_results, columns=['Azimuth', 'Slope', 'Metric'])
            df_grid.to_csv(results_dir / "grid_search_standard.csv", index=False)
            
            try:
                plot_azislope_landscape_2d(
                    df_grid, res_data['opt_azimuth'], res_data['opt_slope'],
                    str(results_dir), "optimization_landscape_2d.png",
                )
                plot_azislope_landscape_3d(
                    df_grid, res_data['opt_azimuth'], res_data['opt_slope'],
                    res_data['opt_metric'], str(results_dir), "optimization_landscape_3d.png",
                )
            ...
            slope_vals = np.linspace(slope_bounds[0], slope_bounds[1], 20)
            points = [(s,) for s in slope_vals]
            ...
            try:
                plot_azislope_ew_1d(
                    slope_vals, metrics, res_data['opt_slope'], res_data['opt_metric'],
                    str(results_dir), "optimization_1d_slope_ew.png",
                )
    ...
    # Header
    print(f"{'Mode':<15} | {'Type':<10} | {'Azimuth':<15} | {'Slope':<10} | {'Metric':<10} | {'Diff':<10}")
    print("-" * 80)
    
    for mode in modes_to_run:
        res = all_results[mode]
        # Precise
        print(f"{mode:<15} | {'Raw':<10} | {res['raw_azimuth']:<15} | {res['raw_slope']:<10.2f} | {res['raw_metric']:<10.2f} | -")
        # Integer
        diff_int = res['opt_metric'] - res['raw_metric']
        print(f"{'':<15} | {'1 deg':<10} | {res['opt_azimuth']:<15} | {res['opt_slope']:<10.0f} | {res['opt_metric']:<10.2f} | {diff_int:<10.2f}")
        # Discrete
        diff_dis = res['discrete_metric'] - res['raw_metric']
        print(f"{'':<15} | {'5 deg':<10} | {res['discrete_azimuth']:<15} | {res['discrete_slope']:<10.0f} | {res['discrete_metric']:<10.2f} | {diff_dis:<10.2f}")
        print("-" * 80)
    ...
    with open(results_txt, 'w') as f:
        ...
        for mode in modes_to_run:
            ...
            f.write("-" * 30 + "\n")
            f.write(f"Raw Slope: {res['raw_slope']:.4f}\n")
            if mode == 'standard':
