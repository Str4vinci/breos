...
2. In 0.6.0, delete `breos/numba_kernels.py` and `_get_numba`. Retain the
   `fast` extra because it now installs the supported compiled dispatch backend.
...
4. Keep Numba optional. The orchestrator has a documented pure-Python default,
   and parity tests cover both execution paths.
...
- If a kernel is kept, a parity test asserts JIT output equals the pure-Python
  ...
  `15min` resolution.
- After removal, the `fast` extra installs only the supported compiled backend.
...
supported production role; its focused tests now cover only compatibility and
the 0.6.0 deprecation contract. It is an independent approximation built from
public documentation, not Polysun or PerMod source code and not a validated
reproduction of the Polysun product.
...
Preserve the exact 0.5.1 release tag and, if the comparison affects published
results, archive that tag or copy the implementation into the article's
versioned research artifact. Its long-term home is the article's repository,
...
## Phase C6 — Prune `tools/` (completed in 0.5.x)

### Outcome

Removed **`tools/analyze_results.py`** (74 lines), which was committed scratch work: bare
...
Removed **`tools/compare_two_results.py`** (93 lines), which was strictly subsumed by
`tools/compare_results.py`, which accepts N folders — including N=2 — with
`--labels` and `--output`. The two-folder script predated it, and no documentation
referenced the obsolete command.

The remaining twelve tools are each distinct. `fetch_weather.py`
...
- **`plot_tariff_comparison`** (`plotting.py:2067`) and
  **`plot_tariff_comparison_manual`** (`plotting.py:2005`).
  ...
  "Time-of-use tariff structures", now targeted for 0.7.0). Keep them, and add
  a module-level comment above `plot_tariff_comparison_manual` saying so, so
  the next dead-code pass does not re-flag them.
...
During 0.5.x (C1-C6 were completed by this point):

1. **C8** — test consolidation. Independent of the public-surface decisions.
2. **C7** — plotting smoke tests for retained exports, establishing coverage
   before any additional retirement decisions.

During 0.6.0:

3. **C1 removal half:** remove `numba_kernels.py` and `_get_numba`. Retain the
   `fast` extra for the supported compiled backend. This removes about 770
   lines.
4. **C2** — Polysun retirement, if confirmed. Self-contained, ~550 lines.
5. **C3** — confirmed uncalled undocumented plots, ~478 lines.
6. **C4** — confirmed orphaned helpers, one PR per module.
7. **C1 rebuild half** — day-kernel implementation, if wanted. Separate PR,
