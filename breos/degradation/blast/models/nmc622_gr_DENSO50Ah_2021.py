...
class Nmc622_Gr_DENSO50Ah_Battery(BatteryDegradationModel):
    ...
    def update_rates(self, stressors):
        ...
        b1t = self.arrhenius(TdegK, p["b1t_Ea"]) * (
            p["b1t_0"]
            + p["b1t_1"] * soc
            + p["b1t_2"] * soc**2
            + p["b1t_3"] * soc**3
            + p["b1t_4"] * soc * TdegK
            + p["b1t_5"] * np.max([0, np.mean(TdegK) - 328.15])
        )
        b1N = np.max(
            [0, p["b1N_0"] * self.arrhenius(TdegK, p["b1N_Ea_1"]) * stress * (1 + p["b1N_1"] * soc) + p["b1N_2"]]
        ) + p["b1N_3"] * self.arrhenius(TdegK, p["b1N_Ea_2"]) * (dod**6)
        ...
        # b1t = np.trapezoid(b1t, x=t_secs) / delta_t_secs
        # b1N = np.trapezoid(b1N, x=t_secs) / delta_t_secs
        # b3t = np.trapezoid(b3t, x=t_secs) / delta_t_secs
        # b3N = np.trapezoid(b3N, x=t_secs) / delta_t_secs
    ...
    def update_states(self, stressors):
        ...
        dq_EFC = self._degradation_scalar * self._update_power_state(
            states["qLoss_EFC"][-1], delta_efc, r["b1t"] * r["b1N"] * p["b1N_4"], 0.5
        )
    ...
    def update_outputs(self, stressors):
        ...
        q = (
            ...
            - states["qLoss_t"][-1]
            - states["qLoss_EFC"][-1]
            - (states["qLoss_BreakIn_t"][-1] + states["qLoss_BreakIn_EFC"][-1])
        )
    ...
    def _extract_stressors(self, t_secs, soc, T_celsius):
        ...
        if np.isnan(Crate):
            ...
            instantaneous_crate[np.abs(instantaneous_crate) < 1e-2] = (
                0  # threshold tiny values to zero to prevent weirdness
            )
            ...
            if len(instantaneous_cchg) > 1:
                Cchg = np.trapezoid(instantaneous_cchg, t_secs_charging) / (t_secs_charging[-1] - t_secs_charging[0])
