...
def fetch_city(city_key: str, city_info: dict, start_year: int, end_year: int,
               source: str, force: bool) -> bool:
    ...
    print(f"Fetching {city_key} ({city_info['name']}) "
          f"from {start_year}-01-01 to {end_year}-12-31 ...")
...
def main():
    ...
    parser.add_argument("--add-locations", action="store_true",
                        help="Add inline-defined cities to configs/base/locations.json")
    ...
    print(f"Cities: {', '.join(cities.keys())}  |  Range: {start_year}-{end_year}"
          f"  |  Source: {source}\n")
