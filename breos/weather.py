...
def parse_weather_filename(filename: str) -> Optional[Dict[str, str]]:
    ...
    match = re.match(
        r'^([a-z]+)_(tmy|historical)_(\d{4})_(\d{4})_([\w-]+)$', name
    )
    if match:
        return {
    ...
    match = re.match(
        r'^([a-z]+)_(tmy|historical)_(\d{4})_([\w-]+)$', name
    )
    if match:
        return {
            'location': match.group(1),
...
def load_weather(
    ...
    if (best['type'] == 'historical' and start_year is not None
...
def fetch_tmy_weather_data(
    ...
    if sample_year is not None:
        ...
        if sample_year % 4 == 0 and (sample_year % 100 != 0 or sample_year % 400 == 0):
            raise ValueError(
                f"Sample year {sample_year} is a leap year. TMY has 8760 hours. Use non-leap year."
            )
        ...
        new_index = pd.date_range(
            start=f"{sample_year}-01-01 00:00",
            end=f"{sample_year}-12-31 23:00",
            freq='h',
            tz=timezone
        )
...
def fetch_weather_data(
    ...
    Args:
        ...
        slope: Tilt of the PV panel (degrees)
    ...
    if freq == '15min' or freq == '15T':
        hourly_dataframe = resample_to_15min(
            hourly_dataframe, method='makima',
...
def resample_to_15min(
    ...
    use_clearsky = (latitude is not None and longitude is not None
                    and len(irrad_col_map) > 0)
    ...
    for col in numeric_df.columns:
        ...
        if use_clearsky and col in irrad_col_map:
            ...
            if method == 'makima':
                ...
                interp_k = interp1d(x_original, k_hourly, kind=method,
            ...
            if method == 'makima':
                ...
                from scipy.interpolate import interp1d
                interp = interp1d(x_original, y_original, kind=method,
                                  fill_value='extrapolate')
    ...
    if non_negative_cols is None:
        ...
        for col in df_15min.columns:
            ...
            if any(x in col.lower() for x in ['irrad', 'radiation', 'tilted',
                                               'terrestrial', 'wind', 'speed']):
...
def csv_hourly_to_15min(
    ...
    try:
        ...
        df_15min = resample_to_15min(
            df, method='makima', non_negative_cols=non_negative_cols,
            latitude=latitude, longitude=longitude
        )
...
def select_random_year_and_replace_datetime(
    ...
    if len(selected_year_data) == 8784:  # Leap year
        feb_29_mask = (
            (selected_year_data['date'].dt.month == 2) & 
            (selected_year_data['date'].dt.day == 29)
        )
