...
TRANSPOSITION_MODELS = ("isotropic", "klucher", "haydavies", "reindl", "king", "perez", "perez-driesse")
...
def derive_dni(d: pd.DataFrame, method: str, solpos: pd.DataFrame | None = None) -> pd.Series:
    ...
    if method == "closure":
        ...
        dni = np.divide(
            ...
            cosz,
            out=np.zeros(len(d)),
            where=cosz > np.cos(np.radians(85.0)),
        )
...
def weather_frame(d: pd.DataFrame, dni_method: str = "closure", solpos: pd.DataFrame | None = None) -> pd.DataFrame:
...
def modelled_poa(
    ...
    model_perez: str = "allsitescomposite1990",
    ...
    opts = resolve_pv_model_options(transposition_model=transposition_model, model_perez=model_perez, albedo=albedo)
    detail = _compute_irradiance_and_cell_temp_detail(
        waligned,
        solarpos,
        surface_tilt=tilt,
        surface_azimuth=azimuth,
        pv_params=ARRAYS["16A"].module,
        model_options=opts,
    )
...
def run_chain(
    ...
    loss_overrides=None,
    dni_method: str = "closure",
    temperature_model: str = "faiman",
    model_perez: str = "allsitescomposite1990",
    albedo: float | None = None,
    diffuse_iam: str = "none",
...
def screen_array_days(d: pd.DataFrame, label: str, model_ac_kw: pd.Series, weather_ok: pd.Series) -> pd.Series:
