...
from nist_validate import (
    LOSS_CASES, MODULE, N_MODULES, SHUNTS, load, metrics, run_chain,
)
...
def clear_sky_poa(index: pd.DatetimeIndex) -> pd.Series:
    ...
    poa = pvlib.irradiance.get_total_irradiance(
        TILT, AZIM, sp["apparent_zenith"], sp["azimuth"],
        cs["dni"], cs["ghi"], cs["dhi"], model="haydavies",
...
def main() -> None:
    ...
    print(f"outage/snow screening: {len(outage)} of {len(ratio)} days excluded, "
        ...
        for poa_col in ("RefCell1_Wm2", "SEWSPOAIrrad_Wm2"):
            for case, overrides in LOSS_CASES.items():
                ...
                for label, mask in (("all days", slice(None)), ("normal days", normal)):
                    ...
                    rows.append({"temp_model": temp_model, "poa": poa_col,
                                 "losses": case, "subset": label, **m})
    ...
    print(sub[["losses", "subset", "model_kWh", "meas_kWh", "bias_%",
               "daily_nRMSE_%", "daily_r"]].round(3).to_string(index=False))
    ...
    for temp_model in ("faiman", "pvsyst-freestanding", "sapm-open-rack-glass-glass"):
        ...
        print(f"  {temp_model:28s} bias={err.mean():+6.2f} C  RMSE={np.sqrt((err**2).mean()):5.2f} C  r={r['temp_cell'][hot].corr(rtd[hot]):.4f}")
    ...
    for name, sel in regimes.items():
        s = sel & keep
        out.append({"regime": name, "days": int(s.sum()),
                    "model_kWh": am[s].sum(), "meas_kWh": dd[s].sum(),
