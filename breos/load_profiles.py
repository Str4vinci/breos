...
from breos.utils import is_leap_year, count_leap_years, get_hours_per_step
...
def load_profile(
    ...
    use_native_15min = (
        freq in ('15min', '15T') and 
        profile_type in PROFILE_FILES_15MIN and
        (rlp_directory / PROFILE_FILES_15MIN[profile_type]).exists()
    )
    ...
    new_index = pd.date_range(
        start=start_date,
        periods=hours_in_year * steps_per_hour,
        freq=native_freq,
        tz=timezone
    )
...
def scale_to_annual_consumption(
    ...
    annual_consumption_kwh: float,
    column: str = 'Electrical Consumption [W]'
) -> None:
    """
