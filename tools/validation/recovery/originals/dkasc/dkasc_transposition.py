...
from dkasc_validate import (
    LOSS_CASES, STEP_H, TRANSPOSITION_MODELS, day_mask, irradiance_metrics, load,
    modelled_poa, run_chain, screen_array_days, screen_weather_days,
    solar_position, weather_frame,
)
...
PEREZ_SETS = ("allsitescomposite1990", "allsitescomposite1988", "sandiacomposite1988",
              "usacomposite1988", "france1988", "phoenix1988", "elmonte1988",
              "osage1988", "albuquerque1988", "capecanaveral1988", "albany1988")
...
def leg_a(d: pd.DataFrame, w: pd.DataFrame, mask: pd.Series) -> pd.DataFrame:
    ...
    for model in TRANSPOSITION_MODELS:
        ...
        rows.append({"transposition": model, "perez_set": "-", "albedo": "default",
    ...
    for pset in PEREZ_SETS:
        ...
        rows.append({"transposition": "perez", "perez_set": pset, "albedo": "default",
                     **irradiance_metrics(poa, d["gti_meas"], mask)})
    for alb in ALBEDOS:
        if alb is None:
            continue
        for model in ("isotropic", "haydavies", "perez"):
            ...
            rows.append({"transposition": model, "perez_set": "-", "albedo": alb,
                         **irradiance_metrics(poa, d["gti_meas"], mask)})
...
def normal_days(d: pd.DataFrame, w: pd.DataFrame, labels) -> pd.Series:
    ...
    for label in labels:
        r = run_chain(d, label, "haydavies", LOSS_CASES["no-availability-no-shading"],
                      weather=w)
...
def pair_drift_measured(d: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    e = pd.DataFrame({c: d[f"P_{c}"].resample("QS").sum() * STEP_H
                      for c in ("16A", "16D", "12", "13")})
...
def pair_drift(d: pd.DataFrame, w: pd.DataFrame) -> pd.DataFrame:
    ...
    for year, sub in d.groupby(d.index.year):
        ...
        for label in ("16A", "16D", "12", "13"):
            r = run_chain(sub, label, "haydavies",
                          LOSS_CASES["no-availability-no-shading"], weather=ws)
...
def leg_b(d: pd.DataFrame, w: pd.DataFrame, loss_case: str, ok_days: pd.Series,
          models=TRANSPOSITION_MODELS, perez_sets=PEREZ_SETS) -> pd.DataFrame:
    ...
    for model, pset in cases:
        ...
        for label in ("16A", "16D"):
            r = run_chain(d, label, transposition_model=model, loss_overrides=overrides,
                          model_perez=pset, weather=w)
...
def decomposition_sweep(d: pd.DataFrame, mask: pd.Series, solpos: pd.DataFrame) -> pd.DataFrame:
    ...
    for dni_method in DNI_METHODS:
        ...
        for model in ("isotropic", "haydavies", "perez"):
            ...
            rows.append({"dni": dni_method, "transposition": model,
                         "POA_bias_%": m["bias_%"], "POA_RMSE_Wm2": m["RMSE_Wm2"], "r": m["r"]})
    return pd.DataFrame(rows)


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--gti-data", type=Path, required=True,
                    help="frame covering a year with good tilted-pyranometer coverage (Leg A)")
    ap.add_argument("--pair-data", type=Path, required=True,
    ...
    print(f"LEG A: {args.gti_data.name}, {int(ok.sum())} of {len(ok)} days with a usable "
    ...
    print(a[a.albedo == "default"][["transposition", "perez_set", "model_kWh_m2",
                                    "meas_kWh_m2", "bias_%", "RMSE_Wm2", "r"]]
    ...
    print(a[a.albedo != "default"][["transposition", "albedo", "bias_%", "RMSE_Wm2"]]
          .round(3).to_string(index=False))
    ...
    print(drift[["year", "days", "bias_16A_%", "bias_16D_%", "bias_12_%", "bias_13_%",
                 "W/N_meas", "W/N_model", "ratio_err_%"]].round(3).to_string(index=False))
