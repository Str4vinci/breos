"""Independent approximation of a publicly documented lifetime method.

This deprecated, article-scoped module reconstructs a simplified Wöhler/Miner
comparison baseline from Vela Solaris's public Polysun battery-lifetime
documentation. It is not Polysun source code, is not derived from PerMod, and
has not been validated as a reproduction of the current Polysun product. BREOS
is not affiliated with or endorsed by Vela Solaris AG.

The public documentation describes 20 equal-width depth-of-discharge bins, a
Wöhler cycle-life curve for lithium-ion batteries, linear damage accumulation,
fixed default calendar lifetimes, and selection of the shorter cycle or
calendar life. This module adds BREOS-specific choices that must not be
presented as results generated by Polysun:

* local-extrema half-cycle counting;
* literature-derived example Wöhler parameters rather than catalogue data;
* repeated use of one annual state-of-charge profile; and
* an illustrative linear ``SOH_Equivalent`` series for legacy plots.

References:
    - Vela Solaris AG, Polysun User Manual, "Battery Lifetime Estimation"
...
class PolysunDegradationConfig:
    """Configuration for the documentation-derived comparison approximation.
    ...
    Attributes:
        ...
        n_bins: Number of DOD histogram bins (the public method uses 20).
...
def compute_dod_histogram(
    ...
    This is a BREOS-specific local-extrema approximation, not a verified copy
    of Polysun's cycle-counting implementation. Half-cycles are binned by DOD
    and each contributes 0.5 full-cycle equivalents.

    Args:
        soc_series: SOC timeseries (0-1 range), typically one year of data.
        n_bins: Number of equal-width DOD bins (the public method uses 20).
...
def predict_polysun_lifetime(
    ...
    """Apply the lifetime aggregation described in the public documentation.
...
def simulate_polysun_degradation(
    ...
    """Run the independent documentation-derived approximation.

    This implementation reuses the same annual SOC profile and does not feed
    degradation back into the energy balance. Damage accumulates linearly;
    when cumulative damage >= 1, the battery is replaced. These implementation
    choices are not claims about the current Polysun product.
    ...
    for year in range(1, n_years + 1):
        ...
        # Legacy illustrative mapping: this approximation does not model
        # continuous SOH, so map damage linearly for the old comparison plot.
