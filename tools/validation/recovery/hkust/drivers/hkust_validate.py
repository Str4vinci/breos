...
def _physical_mask(target: pd.Series, info: dict[str, Any]) -> tuple[pd.Series, float | None]:
    capacities = [
        float(value) for value in (info["inverter_ac_kw"], info["system_rated_kw"]) if value is not None and value > 0
    ]
...
def _site_mapping_row(path: Path, with_optimizer: bool, info: dict[str, Any], freq: str | None) -> dict[str, Any]:
    ...
    for group in groups:
        ...
        qualities.append(
            "direct CEC analog" if model in {"JKM390M_6RL3_TV", "JKM395M_6RL3_V"} else "approximate CEC analog"
        )
    return {
        "site": path.stem,
...
def _score_site(
    ...
    model_weather = weather.loc[target.index.min().floor(freq) : target.index.max().floor(freq)]
...
def _inverter_files_for_site(site_stem: str, paths: list[Path]) -> list[Path]:
    ...
    if _normalise_name(site_stem) == _normalise_name("Indoor Sports Centre"):
        matches = [
            path
            for path in paths
            if _normalise_name(re.sub(r"_Inverter(?:_\d+)?$", "", path.stem)) == _normalise_name("Indoor Sports Centre")
        ]
    return matches
...
def _optimizer_consistency(
    ...
    inverter_series = [
        _read_power_series(path, "totalActivePower(W)").resample("15min", closed="left", label="left").mean()
    ...
    return {
        ...
        "daily_difference_lt_0_1_kwh_pct": float((np.abs(daily_error) < 0.1).mean() * 100)
        if len(daily_error)
        else None,
...
def _aggregate_metrics(
    ...
    for population, arrays in population_actual.items():
        ...
        scoped = [
            ...
            if row.get("status") == "scored" and row.get("panel_level_optimizer") == (population == "with_optimizer")
        ]
        result[population] = {
            "pooled_daylight_metrics": metrics,
            "pooled_daylight_raw_metrics": _finite_metric(actual, np.concatenate(population_raw[population])),
            "macro_mean_test_rmse_W": float(np.mean([row["test_daylight_rmse_W"] for row in scoped]))
            if scoped
            else None,
            "macro_mean_test_raw_rmse_W": float(np.mean([row["test_raw_daylight_rmse_W"] for row in scoped]))
            if scoped
            else None,
            "macro_mean_test_r": float(
                np.mean([row["test_daylight_r"] for row in scoped if row["test_daylight_r"] is not None])
            )
            if scoped
            else None,
    ...
    if all_actual:
        ...
        result["all_scored_sites"]["macro_mean_test_rmse_W"] = float(
            np.mean([row["test_daylight_rmse_W"] for row in scored])
        )
        result["all_scored_sites"]["macro_mean_test_raw_rmse_W"] = float(
            np.mean([row["test_raw_daylight_rmse_W"] for row in scored])
        )
        result["all_scored_sites"]["macro_mean_test_r"] = float(
            np.mean([row["test_daylight_r"] for row in scored if row["test_daylight_r"] is not None])
        )
    return result
...
def _timing_contract() -> dict[str, Any]:
    ...
    hourly_path = sorted((PV_ROOT / "PV stations without panel level optimizer" / "Site level dataset").glob("*.csv"))[
        0
    ]
    hourly = pd.read_csv(
        hourly_path,
    ...
    optimizer_site_path = (
        PV_ROOT / "PV stations with panel level optimizer" / "Site level dataset" / "Indoor Sports Centre.csv"
    )
    ...
    optimizer_inverter = _read_power_series(optimizer_inverter_path, "totalActivePower(W)")
    ...
    for site_path, with_optimizer in _site_paths():
        ...
        components = [_read_power_series(path, "totalActivePower(W)") for path in matching]
        ...
        right_metrics = population_metrics(summed.resample("15min", closed="right", label="right").mean())
        left_metrics = population_metrics(summed.resample("15min", closed="left", label="left").mean())
    ...
    if left_lower_rmse_sites != len(population_rows) or left_higher_r_sites != len(population_rows):
    ...
    population_summary = {
        ...
        "median_right_rmse_W": float(np.median([row["right_label"]["rmse_W"] for row in population_rows])),
        "median_left_rmse_W": float(np.median([row["left_label"]["rmse_W"] for row in population_rows])),
        "median_right_r": float(np.median([row["right_label"]["r"] for row in population_rows])),
        "median_left_r": float(np.median([row["left_label"]["r"] for row in population_rows])),
    ...
    return {
        ...
        "hourly_energy_identity_check": {
            ...
            "max_abs_generation_minus_power_times_one_hour_kWh": float(hourly_residual.abs().max()),
            "mean_abs_generation_minus_power_times_one_hour_kWh": float(hourly_residual.abs().mean()),
        },
...
def main() -> None:
    ...
    if scored_consistency:
        ...
        optimizer_consistency_summary["weighted_daily_difference_lt_0_1_kwh_pct"] = (
            float(
                sum(
                    (row.get("daily_difference_lt_0_1_kwh_pct") or 0) * (row.get("daily_rows", 0) or 0)
                    for row in scored_consistency
                )
                / daily_total
            )
            if daily_total
        ...
        optimizer_consistency_summary["weighted_daily_difference_lt_1_kwh_pct"] = (
            float(
                sum(
                    (row.get("daily_difference_lt_1_kwh_pct") or 0) * (row.get("daily_rows", 0) or 0)
                    for row in scored_consistency
                )
                / daily_total
            )
        ...
        optimizer_consistency_summary["mean_site_daily_mean_abs_difference_kwh"] = float(
            np.mean(
                [
                    row["daily_mean_abs_difference_kwh"]
                    for row in scored_consistency
                    if row["daily_mean_abs_difference_kwh"] is not None
                ]
            )
        )
    ...
    print(
        json.dumps(
            {"aggregate_metrics": aggregate, "optimizer_consistency": optimizer_consistency_summary},
            indent=2,
            sort_keys=True,
            allow_nan=False,
        )
