...
class Nmc111_Gr_Sanyo2Ah_Battery(BatteryDegradationModel):
    ...
    def update_rates(self, stressors):
        ...
        beta_cap = p["qcyc_A_V"] * (V_rms - p["qcyc_B_V"]) ** 2 + p["qcyc_C"] + p["qcyc_D_DOD"] * dod
        beta_res = p["rcyc_A_V"] * (V_rms - p["rcyc_B_V"]) ** 2 + p["rcyc_C"] + p["rcyc_D_DOD"] * dod

        # Calculate time based average of each rate
        alpha_cap = np.trapezoid(alpha_cap, x=t_secs) / delta_t_secs
        alpha_res = np.trapezoid(alpha_res, x=t_secs) / delta_t_secs
    ...
    def update_states(self, stressors):
        ...
        dq_t = self._degradation_scalar * self._update_power_state(
            states["qLoss_t"][-1], delta_t_days, r["alpha_cap"], p["qcal_p"]
        )
        dq_EFC = self._degradation_scalar * self._update_power_state(
            states["qLoss_EFC"][-1], Ah_throughput, r["beta_cap"], p["qcyc_p"]
        )
        # Resistance
        dr_t = self._degradation_scalar * self._update_power_state(
            states["rGain_t"][-1], delta_t_days, r["alpha_res"], p["rcal_p"]
        )
