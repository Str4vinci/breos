...
def _run_single_sim(args_tuple):
    ...
    """
    (loc_key, n_modules, battery_kwh, dc_1mod, temp_series, load_data, costs_cfg, pv_params, years_projection) = (
        args_tuple
    )
...
def run_sweep(
    ...
    if max_workers == 1:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            future_to_idx = {executor.submit(_run_single_sim, job): idx for idx, job in enumerate(jobs)}
...
def _compute_marginal_returns(results_df, locations):
    ...
    for loc in locations:
        ...
        for batt in battery_sizes:
            ...
            results_df.loc[subset.index, "marginal_gi_per_panel"] = subset["year1_grid_independence"].diff().values
...
def generate_outputs(results_df, output_dir, locations, cost_projections=None):
    ...
    if cost_projections:
        ...
        if len(locations) >= 2:
            ...
            print("Saved: cross-location breakeven comparisons")


def main():
    parser = argparse.ArgumentParser(description="Batch compare PV+battery grid independence across locations")
    parser.add_argument(
        "--locations",
        nargs="+",
        default=["porto", "berlin"],
    ...
    for loc_key in args.locations:
        ...
        print(
            f"  {loc_key} load profile: type={profile_type}, {ANNUAL_CONSUMPTION_KWH} kWh/yr, {len(loc_load_data)} steps"
        )
