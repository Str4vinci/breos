...
def loss_ladder(d: pd.DataFrame, w: pd.DataFrame, ok: dict[str, pd.Series]) -> pd.DataFrame:
    ...
    for label in FIXED:
        ...
        for case, overrides in LOSS_CASES.items():
            ...
            rows.append({"array": label, "orientation": "west" if label == "16D" else "north", "losses": case, **mm})
    ...
    for year, sub in d.groupby(d.index.year):
        ...
        for label in FIXED:
            if sub[f"P_{label}"].notna().mean() < 0.5:
                continue
            r = run_chain(sub, label, REFERENCE_MODEL, LOSS_CASES["module-only"], weather=ws)
            ok = screen_array_days(sub, label, r["ac_model_kW"], wok)
            m = day_mask(sub.index, ok)
            mm = metrics(r["ac_model_kW"][m], sub[f"P_{label}"][m])
            rows.append({"year": year, "array": label, "days": int(ok.sum()), "bias_module_only_%": mm["bias_%"]})
    t = pd.DataFrame(rows)
    fits = []
    ...
    for model in ("isotropic", "haydavies", "perez"):
        for max_tilt in (90.0, 75.0, 60.0):
            bd = calculate_pv_production_tracking_breakdown(
                w,
                location(),
                n_modules=arr.n_modules,
                tracking="dual_axis",
                dual_axis_max_tilt=max_tilt,
                pv_params=arr.module,
                freq="5min",
...
def main() -> None:
    ...
    ap.add_argument(
        "--ladder-years", default=None, help="YYYY:YYYY slice used for the loss ladder, regimes and tracker"
    )
    ap.add_argument(
        "--tracker-years",
        default=None,
    ...
    print(per_year.pivot(index="year", columns="array", values="bias_module_only_%").round(2).to_string())
    print()
    ...
    for label in FIXED:
        print(f"  {label:4s} normal on {int(ok[label].sum())} days ({int((wok & ~ok[label]).sum())} array-abnormal)")
    ...
    print(
        ladder[["array", "orientation", "losses", "model_kWh", "meas_kWh", "bias_%", "daily_nRMSE_%", "daily_r"]]
        .round(3)
    ...
    print("  BREOS default DEFAULT_PVWATTS_LOSSES stacks to 14.1 %; NIST Gaithersburg measured 5.35 %.")
