...
class Nmc111_Gr_Kokam75Ah_Battery(BatteryDegradationModel):
    ...
    def __init__(self, degradation_scalar: float = 1, label: str = "NMC111-Gr Kokam"):
        ...
        self.states = {
            ...
            "qLoss_LAM": np.array(
                [1e-8]
            ),  # relative active material change, charge-throughput dependent (electrode damage)
    ...
    def update_rates(self, stressors):
        ...
        q5 = np.trapezoid(q5, x=t_secs) / delta_t_secs
    ...
    def update_states(self, stressors):
        ...
        states = self.states
        # Capacity
        dq_LLI_t = self._degradation_scalar * self._update_power_state(
            states["qLoss_LLI_t"][-1], delta_t_days, r["q1"], p["q2"]
        )
        dq_LLI_EFC = self._degradation_scalar * self._update_power_state(
            states["qLoss_LLI_EFC"][-1], delta_efc, r["q3"], p["q4"]
        )
        dq_LAM = self._degradation_scalar * self._update_sigmoid_state(
            states["qLoss_LAM"][-1], delta_efc, 1, 1 / r["q5"], p["p_LAM"]
        )

        # Resistance
        dr_LLI_t = self._degradation_scalar * self._update_power_state(
            states["rGain_LLI_t"][-1], delta_t_days, p["r1"] * r["q1"], p["r2"]
        )
        dr_LLI_EFC = self._degradation_scalar * self._update_power_state(
            states["rGain_LLI_EFC"][-1], delta_efc, p["r3"] * r["q3"], p["r4"]
        )
