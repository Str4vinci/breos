...
def _clear_sample_mask(w: pd.DataFrame) -> np.ndarray:
    ...
    daily_clearness = w["ghi"].resample("D").sum() / pd.Series(_clearsky(utc), index=w.index).resample("D").sum()
...
def timezone_and_labelling(w: pd.DataFrame) -> None:
    ...
    for name, off in CANDIDATE_OFFSETS.items():
        ...
        best = min(
            _scaled_rmse(ghi[clear], _clearsky(utc + pd.Timedelta(minutes=s))[clear]) for s in np.arange(-5, 5.5, 0.5)
        )
    ...
    print(f"  best shift {best:+.2f} min   (interval-start would be +2.50, interval-end -2.50, centre 0.00)")
    print(
        f"  RMSE at -2.50 / 0.00 / +2.50 min: "
...
def sensor_orientation(w: pd.DataFrame) -> None:
    ...
    coarse = idx[::4]
    best = min(
        ((rmse(t, a, coarse), t, a) for t in np.arange(0.0, 46.0, 5.0) for a in np.arange(0.0, 360.0, 10.0)),
        key=lambda r: r[0],
    )
    _, t0, a0 = best
    best = min(
        (
            ...
            for a in np.arange(a0 - 10, a0 + 10.01, 2.0)
        ),
        key=lambda r: r[0],
    )
    _, t0, a0 = best
    best = min(
        (
            ...
            for a in np.arange(a0 - 2, a0 + 2.01, 1.0)
        ),
        key=lambda r: r[0],
    )
    best = (best[0], best[1], best[2] % 360)
    print(f"  data-driven fit: tilt {best[1]:.1f} deg, azimuth {best[2]:.1f} deg  (RMSE {best[0]:.2f} W/m2)")
...
def main() -> None:
    ...
    ap.add_argument("--year", type=int, default=2016, help="year used for the timing and orientation fits")
    ap.add_argument(
        "--fit-years", type=int, default=3, help="years ending at --year pooled for the interval-labelling fit"
    )
    ap.add_argument(
        "--sections", default="1,2,3,4,5", help="comma-separated subset to run; the timing fits (1,2) are the slow ones"
    )
    args = ap.parse_args()
    pd.set_option("display.width", 220)

    raw = _read(args.raw_dir / f"{WEATHER_STEM}.csv", ["timestamp", *WEATHER_CHANNELS]).rename(columns=WEATHER_CHANNELS)
    w = raw.copy()
    for col, (lo, hi) in LIMITS.items():
    ...
    print(f"timing fits use {args.year - args.fit_years + 1}-{args.year}; orientation fit uses {args.year}")
