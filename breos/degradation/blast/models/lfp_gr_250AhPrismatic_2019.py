...
class Lfp_Gr_250AhPrismatic(BatteryDegradationModel):
    ...
    def update_rates(self, stressors):
        ...
        kcal = np.abs(p["p1"]) * np.exp(p["p2"] / TdegK) * np.exp(p["p3"] * Ua / TdegK)
        kcyc = (p["p4"] + p["p5"] * dod + p["p6"] * Crate) * (np.exp(p["p7"] / TdegK) + np.exp(-p["p8"] / TdegK))
    ...
    def update_states(self, stressors):
        ...
        dq_t = self._degradation_scalar * self._update_power_state(
            states["qLoss_t"][-1], delta_t_days, r["kcal"], p["pcal"]
        )
        dq_EFC = self._degradation_scalar * self._update_power_state(
            states["qLoss_EFC"][-1], delta_efc, r["kcyc"], p["pcyc"]
        )
